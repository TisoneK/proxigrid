...
1. **Check in at the roster the moment you're through the door — before the deep read, before any analysis, before product work; update it before ending.** (Step 3 — sign first, then read the rest of memory; Steps 15–17 at wrap-up. The push claims your codename: whoever's check-in commit lands first keeps the number. An empty board does not prove you're alone — a live row you didn't write means a peer is here, and the two of you coordinate from minute zero, not after both of you have finished reading.)
