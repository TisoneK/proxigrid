...
- **Protocol:** by agent type — local agents → `.context_ledger/core/rules/ai-engineering-protocol-local.md`; cloud/sandbox agents → `.context_ledger/core/rules/ai-engineering-protocol.md`
- **Protocol location:** on disk — vendored in `.context_ledger/core/` (no network fetch needed; version in `.context_ledger/core/VERSION`, last verified in `../core.lock`)
- **Package upstream:** https://github.com/TisoneK/context-ledger.git (public; was `TisoneK/.context` — GitHub redirects the old URL)
- **Since:** 2026-08-26 (paths/prefixes updated to the `.context_ledger/` + `chore(ledger):` names in the core 1.0.6 migration, 2026-09-13 — same standing parameters)
- **Default role:** engineer — unless a session says otherwise; see `.context_ledger/core/roles/`
- **Scope:** general sweep — discovery + review + fix all safe issues
...
- **Commit style:** Conventional Commits with scope; `chore(ledger):` for `.context_ledger/` changes, `docs(review):` for reports
