...
>   `https://github.com/TisoneK/proxigrid.git`, read `.context_ledger/kickoff.md`, follow it."* If the
...
- **Package upstream (core updates + flaw back-ports):** https://github.com/TisoneK/.context_ledger.git
...
```powershell
pwsh -File .context_ledger/core/bin/context-sync.ps1 verify
pwsh -File .context_ledger/core/bin/context-sync.ps1 status
```

- `verify` fails → core was hand-edited or corrupted. Run
  `sh .context_ledger/core/bin/context-sync rollback` (Windows:
  `pwsh -File .context_ledger/core/bin/context-sync.ps1 rollback`), log a flaw in
  `memory/flaws/log.md`, continue on the restored core.
- `status` reports a newer core with the **same MAJOR** → run
  `sh .context_ledger/core/bin/context-sync update` (Windows: the `.ps1` with
...
On Windows use `pwsh -File .context_ledger/core/bin/context-collab.ps1` with the
...
On Windows use `pwsh -File .context_ledger/core/bin/context-gates.ps1` with the
